...
Use your package manager of choice to install `@scxdraw/core` and its peer dependencies.

```bash
yarn add @scxdraw/core
# or
npm i @scxdraw/core
```
...
import { Renderer, TLShape, TLShapeUtil, Vec } from '@scxdraw/core'
...
The `@scxdraw/core` library provides types for most of the event handlers:
...
Assets are used for shared resources, such as serialized images and videos in the `@scxdraw/tldraw` app. If a shape has an `assetId` property, its component will receive the corresponding asset in the component's props. Like shapes, this interface is meant to be extended with additional properties relevant to the type of asset used (e.g. size, duration, url).
...
```ts
// BoxShape.ts
import type { TLShape } from '@scxdraw/core'
...
import * as React from 'react'
import { shapeComponent, SVGContainer } from '@scxdraw/core'
...
import { Utils, TLBounds, TLShapeUtil } from '@scxdraw/core'
...
Need help? Please [open an issue](https://github.com/scxdraw/tldraw/issues/new) for support.
